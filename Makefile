...
.DEFAULT_GOAL := help
.PHONY: help pc classes emnist deep bp alignment test figures experiments lint all \
        fluid hjb routing other-figures other-experiments

help:
	@echo ""
	@echo "  THE TASK"
	@echo "  make classes     PC across different class counts: 2 / 3 / 5 / 10       (~4 min)"
	@echo "  make emnist      PC on EMNIST-Letters, 26 classes                       (~3 min)"
	@echo ""
	@echo "  THE FINDING"
	...
	@echo "  make lint        ruff"
	@echo ""
	@echo "  OTHER  (the In-Fluid-Net work, in other/ -- see other/README.md)"
...
emnist:
	$(PY) train.py --dataset emnist_letters --epochs 8 \
	  --train-subset 20000 --test-subset 5000 --hidden 128 --weight-lr 0.1
...
all: test alignment experiments figures
...
routing:
	$(RUN) other/scripts/routing_task.py

other-figures:
	$(RUN) other/scripts/make_fluid_figures.py

other-experiments:
	$(RUN) other/scripts/run_fluid_experiments.py
